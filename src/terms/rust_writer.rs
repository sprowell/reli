//! Write relision terms as compilable rust.
//!
//! ```text
//!           _ _     _
...
impl TermWriter for RustWriter {
    /// Create a new writer.  You must have a variable holding the relevant
    /// TermFactory instance.  Specify the name of the variable as the
    /// argument to this method.
    fn new(factory_name: &str) -> Self {
        RustWriter { factory_name: factory_name }
    }
    ...
    fn write_locus(&self, dest: &mut io::Write, fact: &TermFactory, locus: &Locus)
    -> io::Result<()> {
        match locus {
            &Locus::Internal => write!(dest, "Locus::Internal"),
            &Locus::Console(l,c) => write!(dest, "Locus::Console({},{})".format(l,c)),
            &Locus::File(f,l,c) => write!(dest, "Locus::File(\"{}\",{},{})".format(escape(f), l, c)),
        }
    }
    ...
    fn write(&self, dest: &mut io::Write, fact: &TermFactory, term: &Arc<Term>) -> io::Result<()> {
        match term.deref() {
            ...
            &Term::SymbolLiteral { ref typ, ref value, ref locus } => {
